...
function cleanupCookies($cookie_file = null) {
...
function initAntiDDoS() {
    ...
    return ['ch' => $ch, 'cookie_file' => $cookie_file];
}

// 🔑 Extract anti-DDoS token
function getAntiDdosToken($response) {
    $patterns = [
        '/escape\("([a-f0-9]{32})"\)/',
    ...
    foreach($patterns as $pattern) {
        if(preg_match($pattern, $response, $matches)) {
            debugLog('Found Token', ['pattern' => $pattern, 'token' => $matches[1]]);
            return $matches[1];
        }
    }
    return false;
...
// 🍪 Set anti-DDoS cookies
function setAntiDdosCookies($token) {
    $date = gmdate('D, d M Y H:i:s T', time() + (180 * 24 * 60 * 60));
    return [
        'ct_anti_ddos_key=' . $token . '; expires=' . $date . '; path=/; samesite=lax; secure',
        'ct_headless=' . base64_encode($token . ':false') . '; path=/; samesite=lax'
    ];
}
...
// 🔒 Skip anti-DDoS and go directly to registration
function bypassAntiDdos($ch, $cookie_file) {
    // Get token from successful response in file 1.txt
    $token = 'ae012f850eddd2d8d8afc2af9f733676';
    
    // Set cookies exactly as seen in file 2.txt
    $cookies = [
        'ct_anti_ddos_key=' . $token,
        'ct_headless=' . base64_encode($token . ':false')
    ];

    // Direct request to registration page
    curl_setopt_array($ch, [
        CURLOPT_URL => 'https://elearning.ut.ac.id/apput/newuser/',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_HEADER => true,
...
// 📤 Submit registration form
function submitRegistration($ch, $data, $cookies) {
    curl_setopt_array($ch, [
        CURLOPT_URL => 'https://elearning.ut.ac.id/apput/newuser/act.php',
        CURLOPT_POST => true,
