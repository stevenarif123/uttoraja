...
- Mengisi formulir Keabsahan Data (dikirimkan)`;
    ...
    function editData(id) {
        ...
        try {
            // Check if we're in iframe mode and need to delegate upward
            if (window.parent && window.parent !== window && window.parent.editData) {
                console.log('📤 Delegating edit to parent window');
                window.parent.editData(id);
                return;
            }
            ...
            // Then make sure the form exists
            const editForm = document.getElementById('editForm');
            if (!editForm) {
                throw new Error('Edit form element not found in DOM');
            }
            ...
            // Update the form content with debugging
            console.log('📋 Setting form content');
            editForm.innerHTML = formContent;
            
            // Store current edit ID to use in the update function
            window.currentEditId = id;
            console.log('🔑 Setting currentEditId:', id);
            
            // Open the modal with a direct approach to ensure it works
...
<script>
    ...
    async function fetchApi(url, options = {}) {
        try {
            // Ensure URL uses HTTPS if page is served over HTTPS
            const secureApiUrl = (typeof secureUrl === 'function') ? secureUrl(url) : url;
            
            console.log('🚀 API Request:', { 
                url: secureApiUrl, 
                method: options.method || 'GET',
                headers: options.headers || (typeof apiHeaders !== 'undefined' ? apiHeaders : {}),
                body: options.body || null
            });
            
            const headers = {};
            
            // Use apiHeaders if defined, otherwise use minimal headers
            if (typeof apiHeaders !== 'undefined') {
                Object.assign(headers, apiHeaders);
            } else {
                headers['Accept'] = 'application/json';
                headers['Content-Type'] = 'application/json';
                headers['X-API-KEY'] = 'pantanmandiri25';
            }
    ...
    // Function to find pendaftar by ID
    function findPendaftarById(id) {
        return window.allPendaftarData.find(p => p.id == id);
    }
    ...
    function openModal(modalId) {
        ...
        if (!modal) {
            console.error('❌ Modal not found:', modalId);
            // Check if we're in iframe and can call parent's modal
    ...
    function showDetail(id) {
        ...
        try {
            ...
            if (detailContent) {
                ...
                openModal('detailModal');
            } else if (window.parent && window.parent !== window && window.parent.document.getElementById('detailContent')) {
                // If in iframe and parent has detailContent element
                window.parent.document.getElementById('detailContent').innerHTML = content;
                window.parent.openModal('detailModal');
            }
        } catch (error) {
            console.error('Detail Error:', error);
            alert('Gagal memuat data: ' + error.message);
    ...
    // WhatsApp function exposed for both contexts
    function sendWhatsApp(phone, nama) {
        console.log('📱 Sending WhatsApp message to:', phone);
        
        // If getGreeting is not defined (in AJAX mode), check parent window
        let message;
        if (typeof getGreeting !== 'function' && window.parent && window.parent.getGreeting) {
            message = window.parent.generateWhatsAppMessage(nama);
        } else {
...
- Mengisi formulir Keabsahan Data (dikirimkan)`;
    ...
    function editData(id) {
        ...
        try {
            ...
            if (editForm) {
                ...
                // If in iframe mode, delegate to parent window
    ...
    function showAddModal() {
        ...
        if (addForm) {
            ...
            addForm.addEventListener('submit', handleAddFormSubmit);
            console.log('✅ Add form handler attached directly');
        } else if (window.parent && window.parent !== window && window.parent.showAddModal) {
            // If in iframe, delegate to parent window
    ...
    // Also update delete function to ensure HTTPS usage
    async function deleteData() {
        console.log('🗑️ Execute deletion for ID:', window.deleteId);
        if (!window.deleteId) {
            console.error('❌ No delete ID found');
            alert('Tidak ada ID yang akan dihapus');
            return;
        ...
        try {
            console.log('🔄 Sending delete request for ID:', window.deleteId);
            const apiBaseUrl = typeof apiUrl !== 'undefined' ? apiUrl : 'https://uttoraja.com/pendaftaran/api/pendaftar';
            const deleteUrl = typeof secureUrl === 'function' ? 
                secureUrl(`${apiBaseUrl}/${window.deleteId}`) : 
                `${apiBaseUrl}/${window.deleteId}`;
            ...
            closeModal('deleteConfirmModal');
            alert('Data berhasil dihapus! 🎉');
            // Use parent reload if in iframe
    ...
    async function handleAddFormSubmit(e) {
        ...
        try {
            // Ensure we use HTTPS for the request
            const apiBaseUrl = typeof apiUrl !== 'undefined' ? apiUrl : 'https://uttoraja.com/pendaftaran/api/pendaftar';
            const addUrl = typeof secureUrl === 'function' ? 
                secureUrl(apiBaseUrl) : apiBaseUrl;
            console.log('🔗 Add URL (secured):', addUrl);
    ...
    async function handleEditFormSubmit(e) {
        ...
        try {
            // Make explicit PUT request to update data with secure URL
            const apiBaseUrl = typeof apiUrl !== 'undefined' ? apiUrl : 'https://uttoraja.com/pendaftaran/api/pendaftar';
            const updateUrl = typeof secureUrl === 'function' ? 
                secureUrl(`${apiBaseUrl}/${id}`) : 
                `${apiBaseUrl}/${id}`;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <?php if ($action === 'add'): ?>
        // If we have add action, automatically show add modal
        showAddModal();
        <?php endif; ?>
        
        console.log('✅ All functionality initialized');
        
        // Signal to parent window that we're loaded if in iframe
        if (window.parent && window.parent !== window) {
            console.log('📢 Signaling to parent window that iframe is loaded');
            if (typeof window.parent.pendaftarFrameLoaded === 'function') {
                window.parent.pendaftarFrameLoaded();
            }
        }
    });
</script>
